...
gsap.to("#page-1>video",{
  scrollTrigger:{
    trigger: '#pade-1>video',
    start:'2% top', //the animation will start when element is 2% of its height from the top of the viewport.
    end: 'bottom top',
    scroller:'#main'  //the element that should be scrolled
  },
  //call back function
  onStart:() =>{
...
// This line creates a GSAP timeline, which is like a container for animations. It allows you to sequence and control multiple animations together.
...
tl3.to('#page-5>#center-page5',{
     top:'-50%'
})

//page-8
// canvas function: is likely meant to be called to initialize the canvas and set up the animation.
...
// This function contains a long list of URLs representing image files that will be used for the animation. (url appears in sequential order)
...
const frameCount = 198; //total no.of frame
...
gsap.to(imageSeq, {
  ...
  // render function is likely responsible for updating the animation based on the new canvas dimensions.
  onUpdate: render,
});
